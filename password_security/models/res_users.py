...
class ResUsers(models.Model):
    ...
    @api.model
    def get_default_public_user(self):
    ...
    def password_match_message(self):
        ...
        if message:
            message = [_("\nMust contain the following:")] + message
    ...
    def _check_password_rules(self, password):
        ...
        password_regex = [
            "^",
            "(?=(?:[^a-z]*[a-z]){" + str(company_id.password_lower) + "})",
            "(?=(?:[^A-Z]*[A-Z]){" + str(company_id.password_upper) + "})",
            "(?=(?:\D*\d){" + str(company_id.password_numeric) + "})",
            "(?=(?:[^\W_]*[\W_]){" + str(company_id.password_special) + "})",
            f".{{{minlength},}}",
            "$",
        ]
        pattern = "".join(password_regex)
    ...
    def notify_reset_password(self):
        """ True == will notify and send reset password instructions via email
            False == Password expiration date is very close will notify user"""
        self.ensure_one()
        if not self.password_write_date:
            return True
        ...
        expiration_date = self.password_write_date + timedelta(days=self.company_id.password_expiration)
        days_until_expiration = (expiration_date - fields.Datetime.now()).days
        days_to_notice = 7

        result = days_until_expiration <= days_to_notice
    ...
    def change_password(self, old_passwd, new_passwd):
        self.env.user._validate_pass_reset()
        # print(self.env.user._validate_pass_reset())
    ...
    def _change_password(self, new_passwd):
        ...
        if new_passwd:
            self.password_write_date = fields.Datetime.now()
        # print("Pumasa sa UNA:", new_passwd," ",self.password_write_date)

        return super(ResUsers, self)._change_password(new_passwd)
    
class ChangePasswordUserInherit(models.TransientModel):
    _inherit = 'change.password.user'

    def change_password_button(self):
        # Call the super method to execute the original logic
        # res = super(ChangePasswordUserInherit, self).change_password_button()

        # Add your custom validation here
        for user in self:
            if not user.new_passwd:
                raise UserError(_("Before clicking on 'Change Password', you have to write a new password."))
            
            pass_min = user.user_id.company_id.password_minimum
            if pass_min > 0:
                # Check if password write_date is available
                if user.user_id.password_write_date:
                    write_date = user.user_id.password_write_date
                    delta = timedelta(hours=pass_min)
                    if write_date + delta > datetime.now():
                        raise UserError(
                        ...
                        user.user_id._change_password(user.new_passwd)
